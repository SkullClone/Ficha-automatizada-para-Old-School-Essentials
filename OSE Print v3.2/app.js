...
function renderWizLanguages() {
  ...
  if (maxExtra > 0) {
    OSE_LANGUAGES.forEach(lang => {
      ...
      btn.onclick = () => {
        if (wizSelectedExtraLangs.includes(lang)) {
          wizSelectedExtraLangs = wizSelectedExtraLangs.filter(l => l !== lang);
          btn.classList.remove('selected');
        } else {
          if (wizSelectedExtraLangs.length >= maxExtra) return;
...
// RENDERIZADO DEL DADO POLIÉDRICO PARA EL ASISTENTE
function getWizDieSVG(faces, currentVal = null) {
  let displayVal = (currentVal !== null && currentVal !== '') ? currentVal : `d${faces}`;
  let p = "";
  let fz = "28";
  let tY = "64";

  if (faces === 4) {
    p = `<polygon points="50,12 92,86 8,86" fill="#111" stroke="#000" stroke-width="4"/><polygon points="50,12 92,86 50,60" fill="#222" opacity="0.3"/>`;
    fz = "26";
    tY = "70";
  } else if (faces === 6) {
    p = `<rect x="10" y="10" width="80" height="80" rx="10" fill="#111" stroke="#000" stroke-width="4"/><line x1="10" y1="10" x2="25" y2="25" stroke="#333" stroke-width="2"/>`;
    fz = "32";
    tY = "62";
  } else { // d8
    p = `<polygon points="50,6 94,50 50,94 6,50" fill="#111" stroke="#000" stroke-width="3"/><polygon points="28,45 72,45 50,94" fill="#222" stroke="#444" stroke-width="2"/><line x1="50" y1="6" x2="28" y2="45" stroke="#444" stroke-width="2"/><line x1="50" y1="6" x2="72" y2="45" stroke="#444" stroke-width="2"/>`;
    fz = "26";
    tY = "72";
  }
  ...
  return `
    <svg viewBox="0 0 100 100" style="width:100%; height:100%; filter:drop-shadow(0 4px 6px rgba(0,0,0,0.4));">
      ${p}
      <text x="50" y="${tY}" fill="#ffffff" font-size="${fz}" text-anchor="middle" font-family="var(--font-heading)" font-weight="bold">${displayVal}</text>
    </svg>
  `;
}
...
function acceptWizLanguages() {
  ...
  setText('wiz-hd-roll-label', `Tirada de 1d${hd} (${cls.toUpperCase()})`);

  const dieInput = document.getElementById('wiz-hp-die');
  dieInput.max = hd;
  dieInput.value = '';
  
  // Actualizar el botón con la forma poliédrica exacta
  const dieBtn = document.getElementById('wiz-hp-die-btn');
  if (dieBtn) dieBtn.innerHTML = getWizDieSVG(hd, null);

  // Chip de CON con color semántico
  const conChip = document.getElementById('wiz-chip-con');
  if (conChip) {
    conChip.innerText = OSE.fS(modCon);
...
function calcWizHPTotal() {
  ...
  if (dieBtn) dieBtn.innerHTML = getWizDieSVG(hd, dieVal);
...
async function acceptWizHP() {
  ...
  calculateArmorClass();
  runAutoCalculations(false);
  setVal('hp-current', getNum('hp-max')); // Asignar PG actuales al máximo
    ...
    cat.i.forEach(item => {
      ...
      div.innerHTML = `
        <div style="flex:1;">
          ...
          <span style="font-size:0.75rem; font-style:italic; color:#444;">Peso: ${item.w} mo</span>
        </div>
...
// ====== BASE DE DATOS DE REGLAS OSE ======
const OSE_SHOP_DB = [
  { cat: "Armaduras", i: [{ n: "Armadura de Cuero", p: 20, w: 200 }, { n: "Cota de Malla", p: 40, w: 400 }, { n: "Armadura de Placas", p: 60, w: 500 }, { n: "Escudo", p: 10, w: 100 }] },
  { cat: "Armas cuerpo a cuerpo", i: [{ n: "Alabarda", p: 7, w: 150 }, { n: "Bastón", p: 2, w: 40 }, { n: "Daga", p: 3, w: 10 }, { n: "Espada a dos manos", p: 15, w: 150 }, { n: "Espada corta", p: 7, w: 30 }, { n: "Espada normal", p: 10, w: 60 }, { n: "Garrote", p: 3, w: 50 }, { n: "Hacha de batalla", p: 7, w: 50 }, { n: "Hacha de mano", p: 4, w: 30 }, { n: "Lanza", p: 3, w: 30 }, { n: "Lanza de caballería", p: 4, w: 120 }, { n: "Martillo de guerra", p: 5, w: 30 }, { n: "Maza", p: 5, w: 30 }, { n: "Pica", p: 5, w: 150 }] },
  { cat: "Armas a distancia", i: [{ n: "Arco corto", p: 25, w: 20 }, { n: "Arco largo", p: 40, w: 30 }, { n: "Ballesta", p: 30, w: 50 }, { n: "Honda", p: 2, w: 20 }, { n: "Jabalina", p: 3, w: 20 }] },
  { cat: "Munición", i: [{ n: "Carcaj", p: 1, w: 0 }, { n: "Flechas (x20)", p: 5, w: 20 }, { n: "Flecha de plata (x1)", p: 5, w: 1 }, { n: "Piedras para honda", p: 0, w: 0 }, { n: "Virotes (x30)", p: 10, w: 30 }, { n: "Caja de virotes", p: 1, w: 0 }] },
  { cat: "Equipo de aventura", i: [{ n: "Aceite (1 frasco)", p: 2, w: 10 }, { n: "Acónito (1 manojo)", p: 10, w: 0 }, { n: "Agua bendita (1 vial)", p: 25, w: 0 }, { n: "Ajo", p: 5, w: 0 }, { n: "Antorchas (x6)", p: 1, w: 120 }, { n: "Clavos de hierro (x12)", p: 1, w: 10 }, { n: "Cuerda (50')", p: 1, w: 50 }, { n: "Espejo de acero", p: 5, w: 0 }, { n: "Estaca de madera y mazo", p: 3, w: 10 }, { n: "Garfio de anclaje", p: 25, w: 40 }, { n: "Herramientas de ladrón", p: 25, w: 10 }, { n: "Linterna", p: 10, w: 30 }, { n: "Manta de invierno", p: 1, w: 10 }, { n: "Martillo (pequeño)", p: 2, w: 10 }, { n: "Mochila", p: 5, w: 20 }, { n: "Odre de agua", p: 1, w: 5 }, { n: "Pala", p: 2, w: 50 }, { n: "Pértiga de madera (10')", p: 1, w: 100 }, { n: "Pico", p: 2, w: 50 }, { n: "Raciones de hierro (7 días)", p: 15, w: 70 }, { n: "Raciones normales (7 días)", p: 5, w: 200 }, { n: "Saco (grande)", p: 2, w: 5 }, { n: "Saco (pequeño)", p: 1, w: 1 }, { n: "Símbolo sagrado (madera)", p: 2, w: 0 }, { n: "Símbolo sagrado (plata)", p: 25, w: 0 }, { n: "Tiza (x10)", p: 1, w: 0 }, { n: "Velas (x10)", p: 1, w: 0 }, { n: "Yesca y pedernal", p: 3, w: 0 }] }
];
...
function openLanguageModal() {
  ...
  OSE_LANGUAGES.forEach(lang => {
    ...
    div.onclick = () => {
      let alreadySelected = modalSelectedLangs.some(m => m.toLowerCase() === lang.toLowerCase());
      if (alreadySelected) {
        modalSelectedLangs = modalSelectedLangs.filter(l => l.toLowerCase() !== lang.toLowerCase());
...
function openShop() {
  ...
  OSE_SHOP_DB.forEach(cat => {
    ...
    cat.i.forEach(item => {
      ...
      div.innerHTML = `<div style="flex:1;"><span style="font-weight:bold; font-family:var(--font-heading); font-size:1.05rem;">${item.n}</span><br><span style="font-size:0.8rem; font-style:italic; color:#444;" class="shop-weight-lbl">Peso: ${item.w} mo</span></div><div style="font-weight:bold; font-size:1.1rem; text-align:right; font-family:var(--font-heading);">${item.p} mo</div>`;
...
async function buyItem(name, price, weight) {
  ...
  if (currentMo >= price) {
    ...
    closeShop();
    await showDialog('COMPRA', `Compraste ${name} por ${price} mo.`);
  } else {
